...
def search():
    ...
    with results_container:
        ...
        for (index, result) in enumerate(docs_with_scores):
            ...
            print(doc.metadata.get('repo_license_link'))
            ...
            with st.expander(f"**{doc.metadata['repo_origin']}**"):
                ...
                with st.container(horizontal=True, key=f"link_container_{index}"):\
                    ...
                    st.button(label="", icon=":material/block:", type="tertiary", help="Exclude this repo from any further searches.", key=f"block_button_{index}")
...
search_input = st.text_input(label= "Search Input",label_visibility="collapsed",placeholder = "What are you looking for?", max_chars = 4000, key="search_value", on_change=search)
